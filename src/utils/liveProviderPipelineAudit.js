...
export function mergeLiveFeedDiagnosticsIntoAudit(coverageAudit = {}, livePipelineAudit = null) {
  ...
  return {
    ...
    prizepicksFetched: Math.max(Number(coverageAudit.prizepicksFetched) || 0, Number(pp.fetched) || 0),
    prizepicksParsed: Math.max(Number(coverageAudit.prizepicksParsed) || 0, Number(pp.parsed) || 0),
    prizepicksUsable: Math.max(Number(coverageAudit.prizepicksUsable) || 0, Number(pp.filtered) || 0),
    underdogFetched: Math.max(Number(coverageAudit.underdogFetched) || 0, Number(ud.fetched) || 0),
    underdogParsed: Math.max(Number(coverageAudit.underdogParsed) || 0, Number(ud.parsed) || 0),
    underdogUsable: Math.max(Number(coverageAudit.underdogUsable) || 0, Number(ud.filtered) || 0),
