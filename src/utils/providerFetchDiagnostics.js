...
export function recordProviderFetchMetrics(label, metrics = {}) {
  ...
  return updateProviderFetchDiagnostics(label, {
    ...
    payloadSize: Number(metrics.payloadSize ?? metrics.responseSize ?? metrics.responseBodyLength ?? 0) || 0,
    parsedPropsCount: Number(metrics.parsedPropsCount ?? 0) || 0,
    rawPropCount: Number(metrics.rawPropCount ?? 0) || 0,
    finalPropsCount: Number(metrics.finalPropsCount ?? 0) || 0,
    timedOut,
    slow,
    timeoutProvider: timedOut || slow ? label : "",
    lastError: metrics.lastError || metrics.failureReason || "",
    failureReason: metrics.failureReason || metrics.lastError || "",
    failureCategory: metrics.failureCategory || "",
    requestUrl: metrics.requestUrl || "",
    requestSent: Boolean(metrics.requestSent),
    responseReceived: Boolean(metrics.responseReceived),
    responseHeaders: metrics.responseHeaders || {},
    captchaDetected: Boolean(metrics.captchaDetected),
    blockedPayloadDetected: Boolean(metrics.blockedPayloadDetected),
  });
}
...
export function buildProviderEntryDiagnostics(entry = {}, result = {}) {
  ...
  return {
    ...
    finalPropsCount: stored.finalPropsCount ?? debug.usablePropsCount ?? diagnostics.validationCount ?? 0,
    timedOut: Boolean(entry.timedOut || stored.timedOut),
    slow,
    timeoutProvider: entry.timedOut || slow ? label : "",
    lastError: stored.failureReason || stored.lastError || entry.statusReason || result?.warnings?.[0] || "",
    failureReason: stored.failureReason || stored.lastError || entry.statusReason || "",
    failureCategory: stored.failureCategory || "",
    lastPhase: stored.lastPhase || "",
    requestUrl: stored.requestUrl || debug.apiUrl || "",
    requestSent: Boolean(stored.requestSent),
    responseReceived: Boolean(stored.responseReceived),
    responseHeaders: stored.responseHeaders || {},
    captchaDetected: Boolean(stored.captchaDetected),
    blockedPayloadDetected: Boolean(stored.blockedPayloadDetected),
